...
import { RunAccountMixerRequest, RunAccountMixerResponse } from '../../proto/api_pb';
import { saveAccountMixerRequestDefaults, getAppConfig } from '../appconfiguration/settingsSlice';
...
class MixerSettingsForm extends React.Component<Props, InternalState> {
				...
				<Card>
					<Card.Header>
						<Row className="mt-3">
							...
							<Col sm={4} className="text-center pt-2">
								...
								<ErrorAlert error={this.props.runAccountMixerError} />
							</Col>
							<Col sm={4}>
								<span className="float-right">
									<Button
										variant="primary"
										disabled={this.props.isAccountMixerRunning}
					...
					<Card.Body>
						<Form
							...
							<fieldset disabled={this.props.isAccountMixerRunning || this.props.runAccountMixerAttempting}>
								<Form.Group as={Row} >
									<Col sm={4}>
	...
	handleFormSubmit(e: React.FormEvent<HTMLFormElement>) {
		...
		this.props.saveAccountMixerRequestDefaults(this.state.accountmixerRequest)
		return false;
	}
...
const mapStateToProps = (state: IApplicationState) => {
	const request = getAppConfig(state).getAccountMixerRequestDefaults() || new RunAccountMixerRequest()
	return {
...
const loadFormFields = (formRef: React.RefObject<any>, obj: RunAccountMixerRequest) => {
