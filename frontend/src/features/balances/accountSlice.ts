...
const accountSlice = createSlice({
    ...
    reducers: {
        ...
        getAccountsSuccess(
            state,
            action: PayloadAction<IndexedWalletAccounts>
        ) {
        ...
        renameAccountSuccess(
            state,
            action: PayloadAction<RenameAccountResponse>
        ) {
            state.renameAccountAttempting = false
...
export const accountNotification: ActionCreator<any> = (
...
export const loadNextAccountAttempt = (
    accountName: string,
    passphrase: string
): AppThunk => {
    return async (dispatch, getState) => {
        ...
        dispatch(nextAddressAttempt())
        try {
            const resp = await LorcaBackend.fetchNextAccount(
                accountName,
                passphrase
            )
...
export const doRenameAccountAttempt = (
    ...
    return async (dispatch, getState) => {
        ...
        try {
            const resp = await LorcaBackend.renameAccount(
