...
class StakeInfoComponent extends React.Component<Props> {
	render() {
		...
		const allTixCount = s.getVoted() + s.getLive() + s.getImmature() + s.getOwnMempoolTix() + s.getMissed()
		return (
			<Card>
				<Card.Body>
					<Row>
						...
						<Col>
							...
							<h6 className="text-muted"><TicketStatusIcon status={TicketStatus.UNMINED} /> In mempool</h6>
						</Col>
						<Col>
							<h2>{s.getImmature()}</h2>
