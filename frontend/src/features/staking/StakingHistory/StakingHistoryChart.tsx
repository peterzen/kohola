...
import { onTimeFrameChanged, getFilteredStakingHistory, getStakingHistoryAsTimeSeries } from "../stakingSlice"
...
export const colorScheme = ["#7ebdb4", "#f6d198", "#f4a548", "#862a5c", "#27496d", "#142850", "#00909e"]
...
class StakingHistoryChart extends React.PureComponent<Props, InternalState> {
	render() {
		...
		const rewardStyles = styler([
			...
			{ key: "roi", color: colorScheme[5], style: "line", strokeWidth: 5 },
		])
				...
				<Col sm={6}>
						...
						>
							<ChartRow height="400">
								...
								<YAxis
									...
									label="Value"
									min={0}
									max={rewardSeries.max("ticketCostCredit_sum")}
									width="80"
									type="linear"
									format="d"
									visible={false}
					...
					<Legend
						...
						categories={[
							...
							{ key: "ticketCostCredit_sum", label: "Ticket price credit", value: 0 },
							{ key: "rewardCredit_sum", label: "Reward credit", value: 0 },
						]}
					/>
				</Col>
				<Col sm={6}>
...
const mapStateToProps = (state: IApplicationState): StateProps => {
	...
	const filter = (value: number) => (args: number[]) => _.filter(args, (v) => v == value)
