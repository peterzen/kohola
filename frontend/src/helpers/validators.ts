...
export function isValidAddress(
    ...
    const err = (code: ValidationErrorCodes) =>
        new AppError(code, ValidationErrors[code])

    address = address.trim()
    if (!address.length) return err(ValidationErrorCodes.ERR_INVALID_ADDR_EMPTY)
    if (address.length < 25)
        return err(ValidationErrorCodes.ERR_INVALID_ADDR_TOOSHORT)
    if (address.length > 36)
        return err(ValidationErrorCodes.ERR_INVALID_ADDR_TOOLONG)
